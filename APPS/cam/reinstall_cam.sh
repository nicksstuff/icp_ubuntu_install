...
helm install --name cam ibm-cam-3.1.0.tgz --namespace services --set arch=amd64 --set global.image.secretName=camsecret --set global.iam.deployApiKey=$camApiKey --set camBPDResources.requests.cpu=500m --set camBPDResources.limits.cpu=600m --set camMongoPV.persistence.existingClaimName=cam-mongo-pv --set camLogsPV.persistence.existingClaimName=cam-logs-pv --set camTerraformPV.persistence.existingClaimName=cam-terraform-pv --set camBPDAppDataPV.persistence.existingClaimName=cam-bpd-appdata-pv --tls
